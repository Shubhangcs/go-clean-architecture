...
type TableCreationHandler struct {
	foodRepoInstance models.FoodRepository
}

func NewTableCreationHandler(foodRepo models.FoodRepository) *TableCreationHandler {
	return &TableCreationHandler{
		foodRepoInstance: foodRepo,
	}
}

func (t *TableCreationHandler) CreateTable(w http.ResponseWriter, r *http.Request) {
	if succ, err := t.foodRepoInstance.CreateFoodTable(); err != nil {
		panic(err)
	} else {
		w.Write([]byte(succ))
	}
